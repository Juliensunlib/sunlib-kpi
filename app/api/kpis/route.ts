...
async function fetchAll() {
  ...
  do {
    const url = `https://api.airtable.com/v0/${base}/${table}?pageSize=100&${fqs}${offset ? `&offset=${offset}` : ''}`
    const res = await fetch(url, { headers: { Authorization: `Bearer ${key}` }, cache: 'no-store' })
    if (!res.ok) throw new Error(`Airtable ${res.status}: ${await res.text()}`)
    const d = await res.json() as { records: unknown[]; offset?: string }
    all.push(...d.records)
    offset = d.offset
  } while (offset)
  return all as Array<{ id: string; fields: Record<string, unknown> }>
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    const filtered = records.filter(r => {
      const f = r.fields
      if (segment !== 'Tous' && str(f[F.SEGMENT]) !== segment) return false
      if (typeInstall !== 'Tous' && str(f[F.TYPE_INSTALLATION]) !== typeInstall) return false
      if (annee && !str(f[F.MOIS_SIGNATURE]).startsWith(String(annee))) return false
      return true
    })

    // Agrégation par mois
    const byMonth = new Map<string, {
      signes: typeof filtered; poses: typeof filtered; f3: typeof filtered
    }>()
    ...
    const isPro = (r: { fields: Record<string, unknown> }) => str(r.fields[F.SEGMENT]) === 'Pro'
